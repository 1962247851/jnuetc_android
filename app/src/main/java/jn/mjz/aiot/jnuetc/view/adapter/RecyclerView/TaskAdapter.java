...
//            Log.e(TAG, "onBindViewHolder: quitSelectMode");
            ...
            //未处理或者与自己有关或者管理员
//            if (MainViewModel.user.getRoot() != 0 || data.getRepairer() == null || data.getRepairer().contains(MainViewModel.user.getName())) {
            dateAndName = String.format("%s %s", DateUtil.getDateAndTime(data.getDate(), " "), data.getName());
//            } else {
//                隐藏个人信息
//                dateAndName = DateUtil.getDateAndTime(data.getDate(), " ");
//            }
